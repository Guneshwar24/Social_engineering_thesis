...
# Enhanced Persuasion Agent that works with the Digital Twin
class EnhancedPersuasionAgent:
    ...
    def respond(self, user_message):
        ...
        # Generate response based on current stage and user profile
    ...
    def _generate_response(self):
        """Generate a response based on current stage and user profile."""
        user_profile = self.digital_twin.user_profile
    ...
    def simulate_response(self, potential_response):
        """Simulate how the user might respond to a potential message."""
        return self.digital_twin.predict_response(potential_response)
    
    def reset(self):
        """Reset the agent and digital twin."""
        self.conversation_history = []
        self.current_stage = "RAPPORT"
        self.link_introduced = False
...
def create_chat_interface():
    ...
    with gr.Blocks(title="Multi-Agent Chat System") as interface:
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    ...
                    simulate = gr.Button("Simulate Response")
            ...
            with gr.Column(scale=2):
                ...
                simulation_result = gr.Textbox(label="Predicted User Response", value="No simulation run yet")
        ...
        def user_input(user_message, history):
            ...
            return "", history, agent.current_stage, "Yes" if agent.link_introduced else "No", \
                   ...
                   objections_str, twin.user_profile["engagement"], "No simulation run yet"
        
        def clear_chat():
            agent.reset()
            return [], "", agent.current_stage, "No", "None", "low", "neutral", "None", 5.0, "No simulation run yet"
        
        def run_simulation(history):
            if not history:
                return "No conversation to simulate"
            
            # Get the last agent message
            last_agent_msg = None
            for i in range(len(history) - 1, -1, -1):
                if history[i][1]:  # This is an agent message
                    last_agent_msg = history[i][1]
                    break
            
            if not last_agent_msg:
                return "No agent message to simulate response to"
            
            # Predict user response
            predicted_response = agent.simulate_response(last_agent_msg)
            return predicted_response
        
        send.click(
            user_input, 
            [msg, chatbot], 
            [msg, chatbot, stage_info, link_status, interests, trust, style, objections, engagement, simulation_result]
        )
        ...
        clear.click(
            clear_chat, 
            [], 
            [chatbot, msg, stage_info, link_status, interests, trust, style, objections, engagement, simulation_result]
        )
