...
#####################################
# 0. Objection Handling Functions   #
#####################################

def detect_objections(user_message, conversation_history, session_id=None):
    """Detect potential objections in user messages using LLM analysis."""
    try:
        ...
        prompt = f"""
        Analyze this user message for potential objections or concerns.
        ...
        Look for:
        1. Direct objections (e.g., "I don't want to", "I'm not interested")
        2. Indirect concerns (e.g., "I'm not sure", "What if...")
        3. Hesitation or uncertainty
        4. Questions that might indicate underlying concerns
        5. Emotional indicators of resistance
        
        Return a JSON object with:
        - has_objection: boolean indicating if an objection was detected
        - objection_type: one of ["direct", "indirect", "hesitation", "question", "emotional", "none"]
        - objection_text: the specific phrase or pattern that indicates the objection
        - confidence: number between 0-1 indicating confidence in detection
        - suggested_response: brief suggestion for how to address the objection
        """
        
        response = metrics_llm.invoke([
            SystemMessage(content="You are an expert at detecting and analyzing user objections. Return ONLY valid JSON."),
            HumanMessage(content=prompt)
        ])
        
        try:
            result = json.loads(response.content.strip())
            print(f"Objection detection result: {result}")
            return result
        except json.JSONDecodeError as e:
            print(f"Error parsing objection detection response: {str(e)}")
            return {
                "has_objection": False,
                "objection_type": "none",
                "objection_text": "",
                "confidence": 0.0,
                "suggested_response": "Continue conversation normally"
            }
            
    except Exception as e:
        print(f"Error in objection detection: {str(e)}")
        return {
            "has_objection": False,
            "objection_type": "none",
            "objection_text": "",
...
def handle_objection(objection_analysis, conversation_history, session_id=None):
    """Generate an appropriate response to handle detected objections."""
    try:
        if not objection_analysis["has_objection"]:
            return None
        ...
        prompt = f"""
        Generate a response to address the user\'s objection.
        
        CONVERSATION HISTORY:
        {formatted_history}
        
        OBJECTION ANALYSIS:
        - Type: {objection_analysis['objection_type']}
        - Text: {objection_analysis['objection_text']}
        - Confidence: {objection_analysis['confidence']}
        
        Guidelines:
        1. Acknowledge the concern without being defensive
        2. Provide relevant information to address the specific objection
        3. Use empathy and understanding
        4. Maintain a natural, conversational tone
        5. Avoid pressuring or pushing back too hard
        6. If appropriate, ask clarifying questions
        7. Keep the response concise and focused
        
        Return a JSON object with:
        - response: the generated response text
        - strategy: brief description of the handling strategy used
        - confidence: number between 0-1 indicating confidence in the response
        """
        
        response = metrics_llm.invoke([
            SystemMessage(content="You are an expert at handling user objections in a natural, empathetic way. Return ONLY valid JSON."),
            HumanMessage(content=prompt)
        ])
        
        try:
            result = json.loads(response.content.strip())
            print(f"Objection handling result: {result}")
            return result["response"]
        except json.JSONDecodeError as e:
            print(f"Error parsing objection handling response: {str(e)}")
            return "I understand your concern. Could you tell me more about what's on your mind?"
            
    except Exception as e:
        print(f"Error in objection handling: {str(e)}")
        return "I understand your concern. Could you tell me more about what's on your mind?"

def process_message_with_objection_handling(user_message, session_id=None):
    """Process a message with integrated objection handling."""
    try:
        if session_id and session_id in session_memories:
            memory = session_memories[session_id]
        else:
        ...
        conversation_history = memory.get("messages", [])
        objection_analysis = detect_objections(user_message, conversation_history, session_id)
        
        if objection_analysis["has_objection"] and objection_analysis["confidence"] > 0.5:
            objection_response = handle_objection(objection_analysis, conversation_history, session_id)
            if objection_response:
                return objection_response
                
        return process_message(user_message, session_id)
        
    except Exception as e:
        print(f"Error in objection handling process: {str(e)}")
        return process_message(user_message, session_id)
...
CONVERSATION_STAGES = {
    ...
    "OBJECTION_HANDLING": {
        ...
        "policy": """
            ...
            TRANSITION CRITERIA:
            - Move to LINK_INTRODUCTION when:
              * All major objections have been addressed
              * User shows positive engagement (>0.6)
              * Trust level is high (>0.7)
              * At least 3 messages in this stage
        """
    },
    "LINK_INTRODUCTION": {
        "id": 5,
...
def analyze_message_with_llm(message: str, conversation_history: list, current_stage: str) -> dict:
    """
    Analyze a message using an LLM to extract key metrics for social engineering detection.
    
    Args:
        message: The message to analyze
        conversation_history: List of previous messages in the conversation
        current_stage: The current stage of the social engineering attempt
        
    Returns:
        Dictionary with engagement metrics, trust indicators, and communication style
    """
    ...
    for i, msg in enumerate(conversation_history[-5:]):  # Use last 5 messages for context
        if isinstance(msg, dict):
            sender = "User" if msg.get("role") == "user" else "System"
    ...
    prompt = f"""
    Analyze the following message in the context of a conversation where social engineering may be occurring.
    ...
    Perform a detailed analysis and provide the following metrics:
    
    1. Engagement Metrics (0.0-1.0 scale):
       - response_length_quality: Quality and appropriateness of message length
       - topic_continuity: How well message maintains topical relevance
       - question_quality: Quality and engagement level of questions
       - personal_disclosure: Level of personal information shared
       - enthusiasm_level: Detected enthusiasm in the message
    
    2. Trust Indicators (0.0-1.0 scale):
       - sentiment_score: Overall sentiment (higher = more positive)
       - authenticity_score: How authentic the message appears
       - engagement_pattern: How natural the engagement pattern seems
       - linguistic_style_match: How well styles match between messages
    
    3. Communication Style (0.0-1.0 scale):
       - formality_level: Level of formality in language
       - emotional_expressiveness: Degree of emotional content
       - conversation_depth: Depth vs. superficiality of content
       - interaction_quality: Overall quality of the interaction
    
    4. Additional Insights:
       - identified_topics: List of main topics detected
       - potential_concerns: List any potential concerns or red flags
       - suggested_improvements: Ways to improve interaction quality
    
    Return a JSON object with these metrics. ONLY return valid JSON, no explanation text.
    Ensure all numeric values are between 0.0 and 1.0.
    """
    
    try:
        # Call the metrics LLM with the analysis prompt
        response = metrics_llm.invoke([
            SystemMessage(content="You are an expert in communication analysis and social engineering detection. Provide precise metrics based on message content."),
            HumanMessage(content=prompt)
        ])
        ...
        try:
            ...
            # Validate the metrics format
            required_sections = ["engagement_metrics", "trust_indicators", "communication_style", "additional_insights"]
            for section in required_sections:
                if section not in metrics:
                    print(f"Missing required section: {section}")
                    return get_default_metrics()
            
            # Ensure numeric values are in range 0.0-1.0
            numeric_sections = ["engagement_metrics", "trust_indicators", "communication_style"]
            for section in numeric_sections:
                for key, value in metrics[section].items():
                    if not isinstance(value, (int, float)) or value < 0 or value > 1:
                        metrics[section][key] = 0.5  # Default to neutral if invalid
            
            return metrics
            
        except (json.JSONDecodeError, ValueError) as e:
            print(f"Error parsing LLM metrics response: {str(e)}")
            # Return default metrics on parsing error
            return get_default_metrics()
            
    except Exception as e:
        print(f"Error getting message analysis from LLM: {str(e)}")
        return get_default_metrics()
...
def analyze_linguistic_engagement(text):
    """Analyze linguistic patterns for engagement assessment.
    This function is a rule-based approach to determine engagement levels.
    Used when LLM analysis fails. Returns score 0-1."""
    if not text or len(text) < 5:
        return 0.3  # Default for short/empty texts
    ...
    # Engagement indicator terms
    engagement_terms = [
        'yes', 'agree', 'interested', 'tell me more', 'fascinating', 
        'interesting', 'curious', 'want to know', 'tell me', 'sure', 
        'absolutely', 'please', 'thanks', 'thank you', 'great'
    ]
    
    engagement_term_count = sum(1 for term in engagement_terms if term in text)
    
    # Disengagement indicator terms
    disengagement_terms = [
        'no', 'not interested', 'busy', 'later', 'another time',
        'don\'t want', 'stop', 'enough', 'boring', 'bored'
    ]
    
    disengagement_term_count = sum(1 for term in disengagement_terms if term in text)
    
    # Feature signals
    signals = {
        'word_count': min(1.0, word_count / 100),  # Normalize to 0-1
        'avg_word_len': min(1.0, max(0, (avg_word_len - 3) / 5)),  # 3-8 letter words
        'avg_sentence_len': min(1.0, max(0, (avg_sentence_len - 3) / 12)),  # 3-15 words
        'question_density': min(1.0, question_count / max(1, sentence_count)),
        'exclamation_density': min(1.0, exclamation_count / max(1, sentence_count)),
        'engagement_term_ratio': min(1.0, engagement_term_count * 0.2),
        'disengagement_term_ratio': min(1.0, disengagement_term_count * -0.3)
    }
    ...
    weights = {
        ...
        'engagement_term_ratio': 0.2,
        'disengagement_term_ratio': -0.2  # Negative weight
    }
    
    weighted_sum = sum(signals[k] * weights[k] for k in signals if k in weights)
    engagement_score = base_score + weighted_sum
...
class DigitalTwinWithMemory:
    ...
    def update_user_biography(self, session_id=None):
        ...
        try:
            ...
            if not session_id and not self.current_user_id:
                print("[BIOGRAPHY] No session ID provided")
                return False
            
            # Handle session ID type
            ...
            user_messages = [msg["content"] for msg in memory["messages"] 
                            if msg.get("role") == "user"]
            
            if len(user_messages) < 2:
                print(f"[BIOGRAPHY] Not enough messages ({len(user_messages)})")
                return False
                
            # Get learning insights and metrics
            learning_insights = self._calculate_learning_insights(session_id)
            accommodation_score = self._analyze_user_accommodation_patterns(session_id)
    ...
    Create a detailed biography that captures:
    1. Communication style (formality, complexity, directness)
    2. Knowledge areas and expertise
    3. Personality traits and behavioral patterns
    4. Decision-making approach
    5. Trust and engagement patterns
    Focus on evidence-based observations from the conversation."""

            user_prompt = f"""Analyze these messages from user '{user_id}':

    {user_content}

    User Metrics:
    - Trust Level: {learning_insights.get('trust_level', 0.5):.2f}
    - Engagement Depth: {learning_insights.get('engagement_depth', 0.5):.2f}
    - Accommodation Score: {accommodation_score:.2f}
    - Message Count: {len(user_messages)}

    Previous Biography:
    {self._user_biographies.get(user_id, {}).get('biography', 'No previous biography.')}

    Create an updated biography (max 200 words) that describes:
    1. Communication style and patterns
    2. Knowledge areas and expertise
    3. Personality traits and tendencies
    4. Trust and engagement patterns
    5. Notable changes from previous biography

    Base your analysis only on concrete evidence from messages and metrics."""
            ...
            biography = biography_result.content.strip() if hasattr(biography_result, 'content') else str(biography_result).strip()
            
            # Update biography storage
            ...
            # Store biography with metadata
            self._user_biographies[user_id].update({
                "biography": biography,
                "interaction_count": len(user_messages),
                "last_updated": datetime.datetime.now().isoformat(),
                "last_metrics": learning_insights,
                "biography_history": self._user_biographies[user_id].get("biography_history", []) + [{
                    "biography": biography,
                    "timestamp": datetime.datetime.now().isoformat(),
                    "metrics": learning_insights
                }]
            })
            
            print(f"[BIOGRAPHY] Updated for user {user_id}")
            return True
            
        except Exception as e:
            print(f"[BIOGRAPHY] Error: {str(e)}")
            traceback.print_exc()
            return False
            
    def _calculate_learning_insights(self, session_id=None):
        """Calculate learning insights based on prediction history and user interactions."""
        try:
            # Get session-specific memory
            memory = session_memories.get(session_id, conversation_memory)
            
            # Extract user messages
            user_messages = [msg["content"] for msg in memory["messages"] if msg.get("role") == "user"]
            
            # Get trust metrics
            trust_data = self._get_current_trust_metrics()
            
            # Get accommodation patterns
            accommodation_data = self._analyze_user_accommodation_patterns(session_id)
            
            # Get engagement metrics
            engagement_depth = calculate_engagement_depth(user_messages[-1] if user_messages else "", memory["messages"])
            substantive_ratio = calculate_substantive_ratio(memory["messages"])
            
            # Get communication style analysis
            if len(user_messages) >= 3:
                communication_style = analyze_user_communication_style(user_messages)
            else:
                communication_style = {"style": "unknown", "formality": 0.5, "complexity": 0.5}
            
            # Format insights
            insights = {
                "message_count": len(user_messages),
                "trust_level": trust_data.get("trust_level", 0.5),
                "engagement": {
                    "depth": engagement_depth,
                    "substantive_ratio": substantive_ratio
                },
                "communication_style": communication_style,
                "accommodation": accommodation_data.get("accommodation", 0.5),
                "timestamp": datetime.datetime.now().isoformat()
            }
            
            # Store insights in session data
            if "learning_insights" not in self.session_data:
                self.session_data["learning_insights"] = []
            
            self.session_data["learning_insights"].append(insights)
            
            print(f"[LEARNING INSIGHTS] Generated for session {session_id}")
            return insights
            
        except Exception as e:
            print(f"Error calculating learning insights: {str(e)}")
            traceback.print_exc()
            return {"error": str(e)}
...
class DigitalTwinWithMemory:
    ...
    def set_user_for_session(self, user_id):
        ...
        try:
            ...
            # Initialize biography with more detailed structure
            if user_id not in self._user_biographies:
                self._user_biographies[user_id] = {
                    "first_seen": datetime.datetime.now().isoformat(),
                    "biography": "New session, no information available yet.",
        ...
        try:
            ...
            if isinstance(last_updated, str) and last_updated != "unknown":
                try:
                    ...
                    print(f"Warning: Failed to parse timestamp {last_updated}: {str(e)}")
...
def needs_refinement(initial: str, predicted: str, threshold: float = 0.3) -> bool:
    """
    Determines if an assistant response needs refinement based on predicted user reaction.
    Lower threshold (0.3) means higher similarity required before refinement is triggered.
    
    Args:
        initial: The initial response
        predicted: The predicted user reaction
        threshold: Similarity threshold below which refinement is needed
...
def process_message(message, session_id=None):
    """Process a new message and update the digital twin's understanding."""
    try:
        # Input validation
        if len(message.strip()) <= 1 and not message.strip().isalnum():
            print("[VALIDATION] Message too short")
            return "..."
        ...
        # Get prediction and initial response
        prediction = digital_twin.predict_user_response(formatted_history, session_id)
        initial_response = get_llm_response(message, formatted_history, current_stage)
        
        # Refinement process
        final_response = initial_response
        if needs_refinement(initial_response, prediction, threshold=0.3):
            user_messages = [msg for msg in memory["messages"] if msg["role"] == "USER"]
            style_guidance = analyze_user_communication_style(user_messages)
            
            # Refinement logic here...
            # (Include your refinement implementation)
            
        # Process dynamic links
        link_stages = ["LINK_INTRODUCTION", "LINK_REINFORCEMENT", "SESSION_COMPLETION"]
        if current_stage in link_stages and contains_dynamic_link(final_response):
            final_response = process_dynamic_links(final_response, current_stage, session_id)
        
        # Update metrics and biography
        update_metrics_history(message, final_response, session_id)
        
        # Biography updates
        message_count = len([msg for msg in memory["messages"] if msg["role"] == "USER"])
        if message_count % 5 == 0:  # Every 5 messages
            digital_twin.update_user_biography(session_id)
            
        # Add response to memory
        add_assistant_message(final_response, memory, session_id)
        ...
        digital_twin.learn_from_prediction(
            ...
            was_refined=needs_refinement(initial_response, prediction),
            session_id=session_id
        )
...
def update_stage_display(current_stage):
    ...
    all_stages = [
        ...
        {"id": 4, "name": "OBJECTION_HANDLING", "display": "Objection Handling", "description": "Addressing potential concerns and objections before link introduction"},
        {"id": 5, "name": "LINK_INTRODUCTION", "display": "Link Introduction", "description": "Introducing resources naturally"},
        {"id": 6, "name": "LINK_REINFORCEMENT", "display": "Link Reinforcement", "description": "Reinforcing resource value"},
        {"id": 7, "name": "SESSION_COMPLETION", "display": "Session Completion", "description": "Wrapping up conversation"}
    ]
    ...
    progress_percentage = ((current_position - 1) / 7) * 100  # 7 is max steps (8 stages, but 7 transitions)
    
    # Generate HTML for stage indicator
    html = f'<div class="stage-container"><h4>Current Stage: {current_stage}</h4>'
    html += f'<div class="stage-progress-text">Progress: {current_position}/8 (Stage {current_position}, {progress_percentage:.1f}%)</div>'
...
def process_and_update(user_message, chat_history, state):
    ...
    try:
        ...
        # Process the message with objection handling
        response = process_message_with_objection_handling(user_message, session_id)
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    def determine_next_stage(current_stage, user_input, influencer_response, click_detected=False, session_id=None):
        ...
        try:
            ...
            - TRUST_DEVELOPMENT → OBJECTION_HANDLING requires sustained engagement (>0.4), resource interest (>0.4) and >7 messages
            - OBJECTION_HANDLING → LINK_INTRODUCTION happens when a link is introduced
            - LINK_INTRODUCTION → LINK_REINFORCEMENT happens after link interaction
            ...
            if current == "INITIAL_ENGAGEMENT" and messages_count >= 3 and trust_score > 0.3:
                ...
                try:
                    ...
                    if personal_disclosure > 0.2 and trust_score > 0.3:
            ...
            elif current == "OBJECTION_HANDLING":
                # Use resource interest to determine if ready for link introduction
                if resource_interest > 0.4 and messages_count >= 7 and trust_score > 0.4:
                    print("Force progressing from OBJECTION_HANDLING due to resource interest")
                    return "LINK_INTRODUCTION"
                # Secondary fallback to message count
                elif messages_count >= 12:
                    print("Force progressing from OBJECTION_HANDLING due to message count")
                    return "LINK_INTRODUCTION"
...
def analyze_engagement_patterns():
    ...
    lengths = [len(msg["content"].split()) for msg in user_msgs[-5:]]
    if len(lengths) >= 3:
        trend = (lengths[-1] - lengths[0]) / len(lengths)
        return min(0.9, max(0.1, 0.5 + trend * 0.1))
    return 0.5
...
def get_llm_response(user_message, formatted_history, current_stage, additional_instructions=None):
    ...
    # Extract just the most recent user message for focused response
    recent_messages = formatted_history.split('\n')
    last_user_message = ""
    for i in range(len(recent_messages)-1, -1, -1):
        if recent_messages[i].startswith("USER:"):
            last_user_message = recent_messages[i][5:].strip()
            break
    
    # Enhanced conversational continuity instructions based on stage
    continuity_instructions = """
    IMPORTANT CONVERSATIONAL CONTINUITY GUIDELINES:
    - Directly address the user\'s latest message
    - Reference specific words or concepts they mentioned
    - Don't assume information they haven't shared
    - Maintain a clear conversational thread
    - Ask questions related to what they've shared
    """
    
    # Early stage brevity instructions
    brevity_instructions = ""
    if current_stage in ["INITIAL_ENGAGEMENT", "RAPPORT_BUILDING"]:
        brevity_instructions = """
        RESPONSE LENGTH GUIDELINES:
        - Keep responses brief (2-3 sentences)
        - Be concise and to the point
        - Focus on one clear message or question
        - Avoid long explanations unless the user has asked for details
        """
    
    # Create system prompt with enhanced conversational guidance
    system_prompt = f"""You are a conversational assistant engaging with a user.
...
Your goal is to create a natural, flowing conversation that feels personalized.
Focus on creating a response that directly builds on what the user just said.
    """
    
    # User prompt focuses on latest message for better continuity
    user_prompt = f"""
CONVERSATION HISTORY:
{formatted_history}
...
Respond to this specific message in a way that acknowledges what they've just said.
"""
    ...
    # Enforce brevity in early stages
    if current_stage in ["INITIAL_ENGAGEMENT", "RAPPORT_BUILDING"]:
        words = final_response.split()
        if len(words) > 100:
            print(f"Initial response too long ({len(words)} words), truncating to 100 words")
            final_response = " ".join(words[:100])
