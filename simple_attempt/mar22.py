...
10. Balance questions with statements and observations - avoid interview-style conversations that rely too heavily on questions.
11. Share your own opinions and perspectives occasionally to create a natural exchange rather than just extracting information.
12. Use a mix of open-ended questions, statements, observations, and brief personal views to create a dynamic conversation flow.
13. After asking 2-3 questions in sequence, make sure to include statements and observations before asking another question.
14. When the user shows interest in a topic, offer your own thoughts before asking another question.
15. MAINTAIN TOPIC FOCUS - Stay on one topic for at least 2-3 exchanges before transitioning to a new one.
16. LIMIT QUESTIONS - Ask only one question per response to give the user space to think and respond.
17. MATCH THE USER'S PACE - If they give short replies, they may need more time or be losing interest.
18. PRIORITIZE DEPTH OVER BREADTH - Explore fewer topics more deeply rather than many topics superficially.
...
class DigitalTwinWithMemory:
    ...
    def add_to_session_memory(self, context, prediction, actual_response=None):
        ...
        # Trigger biography update after every few messages
        if len(self.custom_session_memory) % 3 == 0 and len(self.custom_session_memory) >= 3:
    ...
    def update_user_biography(self, conversation_history=None):
        ...
        Biography updates after every 3 messages to avoid too frequent changes."""
        try:
            ...
            # Only update after every 3 user messages, or force update if conversation_history is provided
            should_update = (user_message_count % 3 == 0 and user_message_count > 0) or conversation_history is not None
...
USER MESSAGES:
{json.dumps(user_inputs[-10:] if len(user_inputs) > 10 else user_inputs)}
...
IMPORTANT GUIDELINES:
1. Avoid repeating the same questions or topics from previous messages
2. Be attentive to the user's specific interests and engagement level
3. Make your response feel natural and conversational
4. If the user is giving very short responses, try a different approach to engage them
5. BALANCE QUESTIONS WITH STATEMENTS - Don't just ask questions repeatedly
6. Share personal observations or opinions to create a more natural exchange
7. After 2-3 questions in a row, take a break from questions and offer thoughts or observations
8. Make occasional statements about the topic instead of always asking for more information
9. REDUCE TOPIC JUMPING - Stay focused on the current topic for at least 2-3 exchanges before introducing a new one
10. SLOW DOWN THE PACE - Don't introduce multiple potential topics in a single message
11. ASK ONLY ONE QUESTION PER RESPONSE - This gives the user space to think and respond properly
...
def update_digital_twin_actual_response(actual_user_response):
    ...
    # Force biography update after receiving actual response
    if len(digital_twin.custom_session_memory) >= 3:
