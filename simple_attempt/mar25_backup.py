...
def process_message(user_input, session_id=None):
    """Process a user message through the two-agent system using client-specific session ID."""
    # Skip very short messages or just punctuation
    if not user_input or len(user_input.strip()) < 2:
        return "I'm not sure I understand. Could you please say more?"
    
    # Get the appropriate conversation memory
    if session_id and session_id in session_memories:
        current_memory = session_memories[session_id]
    else:
        # Create a new session memory if needed
        if session_id:
    ...
    digital_twin = digital_twin_manager.get_twin_for_session(session_id) if session_id else digital_twin_manager.default_twin
    
    # Get conversation stage and history for context
    current_stage = current_memory.get("current_stage", "INITIAL_ENGAGEMENT")
    stage_policy = CONVERSATION_STAGES[current_stage]["policy"]
    
    # Get previous messages for context
    recent_messages = current_memory["messages"][-5:] if len(current_memory["messages"]) >= 5 else current_memory["messages"]
    ...
    # Process the message with the LLM and return the response
    # (rest of the function remains mostly the same)
    
    # Update conversation memory
    current_memory["messages"].append({
        "role": "INFLUENCER",
        "content": final_message,
        "timestamp": datetime.datetime.now().isoformat(),
        "response_time": response_time,
        "metrics": {
            "linguistic_matching": linguistic_matching,
            "trust_level": metrics["trust_level"],
            "engagement_depth": metrics["engagement_depth"],
            "refined": needs_refine if 'needs_refine' in locals() else False
        }
    })
...
# Add this near the AUTO_SCROLL_JS variable
CLIENT_STORAGE_JS = """
...
function storeConversationState(state) {
    const sessionId = getOrCreateSessionId();
    saveToLocalStorage('conv_state_' + sessionId, state);
    return sessionId;
}

// Retrieve conversation state from localStorage
function getConversationState() {
    const sessionId = getOrCreateSessionId();
    return getFromLocalStorage('conv_state_' + sessionId) || {"conv": [], "session_id": sessionId};
}

// Save the full conversation data at the end of the session
function saveConversationData(data) {
    const sessionId = getOrCreateSessionId();
    saveToLocalStorage('final_data_' + sessionId, data);
    return sessionId;
}
"""

# Add function to retrieve client-side state
def initialize_from_client(client_session_id):
    """
    Initialize the conversation state from client-side session ID.
    This function is called when the Gradio interface loads to restore any existing session.
    """
    if not client_session_id:
        # Generate a new session ID if none provided
        new_session_id = f"session_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}_{random.randint(1000, 9999)}"
        print(f"Created new session ID: {new_session_id}")
        return {"conv": [], "session_id": new_session_id}
    
    # Use the provided session ID to initialize state
    # Note: The actual conversation data will be loaded from localStorage on the client side
    print(f"Initializing from existing client session: {client_session_id}")
    return {"conv": [], "session_id": client_session_id}

# Define key functions needed by the Gradio UI
def process_and_update(user_message, chat_history, state):
    ...
    if not session_id:
        # Try to get it from the hidden input field first
        try:
            # This won't work directly, but we'll handle that in the next step
            session_id = None
        except:
            # Generate a new session ID if we can't get it from the UI
        ...
        print(f"Initialized new session ID: {session_id}")
    
    try:
        # Process the message using the session ID
        response = process_message(user_message, session_id)
        
        # Get the current memory for this session
        if session_id in session_memories:
            current_memory = session_memories[session_id]
        else:
            # If no memory exists yet, create it
            session_memories[session_id] = create_conversation_memory_for_session(session_id)
            current_memory = session_memories[session_id]
        
        # Get current stage from memory
        current_stage = current_memory.get("current_stage", "INITIAL_ENGAGEMENT")
        ...
        has_link = any([
            "https://" in response,
            "http://" in response,
            "youtube.com" in response
        ])
        
        # Add JavaScript to store the updated conversation in localStorage
        js_update = f"""
        <script>
        (function() {{
            const sessionId = "{session_id}";
            const state = {json.dumps({"conv": state["conv"], "session_id": session_id})};
            saveToLocalStorage('conv_state_' + sessionId, state);
            console.log("Updated localStorage for session", sessionId);
        }})();
        </script>
        """
        
        # Create debug info with the has_link variable
        ...
        return (
            ...
            update_stage_display("INITIAL_ENGAGEMENT"),
            gr.update(value="", interactive=True)
        )
...
def add_user_message(user_message, chat_history, state):
    ...
    track_response_timestamp(current_memory)
...
with gr.Blocks(theme=gr.themes.Soft(), js=CLIENT_STORAGE_JS) as demo:
    gr.Markdown("# Enhanced Two-Agent Persuasive System with Custom JSON Memory")
    
    # Add custom JavaScript event handlers for session management
    gr.HTML("""
    <script>
        ...
        window.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById("client_session_id").value = sessionId;
            
            // Set up periodic state saving (every 5 seconds)
            setInterval(function() {
                const chatItems = document.querySelectorAll('.message');
                if (chatItems.length > 0) {
                    // Extract conversations from DOM
                    const conversations = [];
                    chatItems.forEach(function(item) {
                        const role = item.classList.contains('user') ? 'USER' : 'SYSTEM';
                        const content = item.textContent.trim();
                        conversations.push({role: role, content: content, timestamp: new Date().toISOString()});
                    });
                    
                    // Save to localStorage
                    saveToLocalStorage('conv_messages_' + sessionId, conversations);
                }
            }, 5000);
            
            // Load existing conversation if available
            const existingConversation = getFromLocalStorage('conv_messages_' + sessionId);
            if (existingConversation && existingConversation.length > 0) {
                console.log("Found existing conversation with " + existingConversation.length + " messages");
                
                // TODO: Implement a way to restore the chat UI with the existing conversation
                // This would require modifying the Gradio chatbot element via JavaScript
                // or sending the data to the server to handle
            }
        });
    </script>
    """)
    ...
    client_session_id = gr.Textbox(visible=False, value="", elem_id="client_session_id")
    
    # Initialize state with empty value - we'll set it properly in the first function call
    conversation_state = gr.State(value={"conv": []})
    
    # Add a function to set the session ID on page load
    def init_session_state(session_id_value):
        """Initialize the conversation state with a session ID"""
        if not session_id_value:
            # Generate a new session ID if none provided
            new_session_id = f"session_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}_{random.randint(1000, 9999)}"
            return {"conv": [], "session_id": new_session_id}
        return {"conv": [], "session_id": session_id_value}
    ...
    # Add a JavaScript function to retrieve client state
    gr.HTML("""
    <script>
        // Function to initialize the conversation state from localStorage
        function initializeClientState() {
            const state = getConversationState();
            document.getElementById("client_session_id").value = state.session_id || "";
            return state;
        }
    </script>
    """)
    
    # Add a JavaScript function to save final data when leaving
    gr.HTML("""
    <script>
        ...
        window.addEventListener('beforeunload', function() {
            ...
            if (chatItems.length > 0) {
                ...
                // Save final data
                saveConversationData(finalData);
                
                // Signal for final server-side storage if connection is still open
                try {
                    ...
                    console.log("Error sending final save signal:", e);
                }
            }
        });
    </script>
    ...
    gr.HTML("""
    <script>
        // Initialize session ID when the page loads
...
# Add a server-side API endpoint to store the final conversation data
@demo.queue(api_name="save_conversation")
def api_save_conversation(session_id, final_data=None):
    """API endpoint to save final conversation data from client-side storage."""
    try:
        if not session_id:
        ...
        session_folder = os.path.join(STORAGE_DIR, f"{session_id}")
        if not os.path.exists(session_folder):
            os.makedirs(session_folder)
        
        # Set filename
        main_filename = os.path.join(session_folder, "conversation_final.json")
        ...
        with open(main_filename, 'w') as f:
            json.dump({
                "session_id": session_id,
                "timestamp": timestamp,
                "final_data": final_data if final_data else True,
                "storage_time": datetime.datetime.now().isoformat()
            }, f, indent=2)
        
        print(f"Stored final data for session {session_id} at {main_filename}")
        return {"status": "success", "message": f"Data saved for session {session_id}"}
